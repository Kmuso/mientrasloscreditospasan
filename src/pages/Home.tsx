...
export default function Home() {
  ...
  return (
    ...
    <div className="bg-fondo text-texto selection:bg-cine-red selection:text-fondo font-sans transition-colors duration-700">
      ...
      <section className="relative h-[100vh] w-full flex flex-col items-center justify-center p-6 md:p-12 overflow-hidden">
        <motion.main variants={containerVars} initial="initial" animate="animate" className="text-center flex flex-col items-center">
          <div className="overflow-hidden py-2">
            {/* TÍTULO HERO: Usamos el nuevo color dinámico 'text-titulo' */}
            <motion.h1 variants={wordVars} className="text-[4vw] leading-[0.8] font-serif font-bold tracking-[-0.05em] text-titulo transition-colors duration-700">
              mientrasloscréditospasan.com
            </motion.h1>
          </div>
          <div className="overflow-hidden mt-6">
            <motion.p variants={wordVars} className="font-mono text-[10px] md:text-xs uppercase tracking-widest opacity-80 max-w-lg leading-relaxed text-texto transition-colors duration-500">
      ...
      <section ref={videoContainerRef} className="h-[150vh] w-full relative py-12 px-4 md:px-0 flex flex-col items-center">
        <div className="sticky top-0 h-screen w-full flex items-center justify-center overflow-hidden py-12">
          {/* FONDO DEL VIDEO */}
          <motion.div style={{ scale, borderRadius }} className="w-full max-w-[95vw] md:max-w-[80vw] h-full bg-cine-blue relative overflow-hidden shadow-2xl transition-colors duration-700">
            <video autoPlay loop muted playsInline className="absolute inset-0 w-full h-full object-cover opacity-60 img-theme-aware transition-[filter] duration-700" src="https://www.w3schools.com/html/mov_bbb.mp4" />
            <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
              <h2 className="font-serif font-black text-fondo text-[4vw] md:text-[3vw] mix-blend-overlay opacity-90 uppercase tracking-tighter">
                Archivo Visual.
              </h2>
            </div>
          </motion.div>
      ...
      <section className="w-full py-24 px-6 md:px-12 max-w-6xl mx-auto">
        <div className="flex flex-col mb-16 gap-8">
          ...
          <h2 className="font-serif text-[7vw] md:text-[3.5vw] leading-[1.1] tracking-tight max-w-3xl text-titulo transition-colors duration-700">
            Últimas anotaciones del archivo audiovisual.
          </h2>
          <Link to="/ensayos" className="bg-cine-red text-white px-6 py-2 rounded-full text-sm font-semibold hover:scale-105 transition-all self-start shadow-sm">
            Explorar Archivo
          </Link>
        </div>
        
        <div className="flex flex-col border-t border-texto/20 transition-colors duration-700">
          {ultimasEntradas.map((entrada) => (
            <Link key={entrada.id} to="/ensayos" className="group grid grid-cols-1 md:grid-cols-12 gap-2 md:gap-8 items-start md:items-center py-6 border-b border-texto/20 hover:bg-cine-green hover:text-white transition-colors duration-500 px-6 -mx-6 cursor-pointer">
              {/* TÍTULO DE ENSAYO: Usamos 'group-hover:text-white' para contraste, 
                  pero por defecto 'text-titulo' para que cambie de rojo a verde.
              */}
              <h3 className="md:col-span-5 font-serif font-medium text-xl md:text-2xl tracking-tight text-titulo group-hover:text-white group-hover:translate-x-2 transition-all duration-300">
                {entrada.title}
              </h3>
        ...
        <div className="mt-8">
          <Link to="/ensayos" className="text-sm font-medium text-cine-gold hover:opacity-50 transition-opacity flex items-center gap-2 w-max">
      ...
      <section className="w-full py-32 overflow-hidden bg-cine-sand transition-colors duration-700">
        <div className="max-w-7xl mx-auto px-6 md:px-12 mb-16">
          {/* TÍTULO DE SECCIÓN: Usamos 'text-titulo' */}
          <h2 className="font-serif text-[7vw] md:text-[3.5vw] leading-[1.1] tracking-tight max-w-3xl mb-6 text-titulo transition-colors duration-700">
        ...
        <div className="flex gap-4 md:gap-8 px-6 md:px-12 xl:px-[calc((100vw-80rem)/2)] overflow-x-auto snap-x snap-mandatory pb-16 pt-4 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
          {ilustraciones.map((ilus) => (
            ...
            >
              <img src={ilus.img} alt={ilus.title} className="w-full h-full object-cover group-hover:scale-110 transition-[transform,filter] duration-[1.5s] img-theme-aware" />
      ...
      {/* SECCIÓN 5: FOOTER */}
      <footer className="w-full pt-16 pb-12 px-6 md:px-12 xl:px-20 border-t border-texto/10 transition-colors duration-700">
        <div className="flex flex-col justify-between min-h-max max-w-7xl mx-auto w-full">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-8 flex-grow">
            ...
            <div className="flex flex-col justify-between h-full lg:col-span-7">
              <p className="font-mono text-[10px] uppercase tracking-widest text-texto opacity-50 mb-12 max-w-xs transition-colors">
                ...
                <nav className="flex flex-col gap-0 mb-12">
                  {mainLinks.map((item) => (
                    <Link 
                      key={item}
                      to={`/${item.toLowerCase()}`}
                      // HOVER MENÚ: Usamos el nuevo 'text-titulo' para que el hover también sea dinámico
                      className="font-serif font-medium text-[12vw] md:text-[6.5vw] lg:text-[5vw] tracking-tight leading-[1.1] text-texto hover:text-titulo transition-colors block duration-500"
                    >
                      {item}
                    </Link>
                  ))}
                ...
                <div className="flex flex-wrap items-center gap-6 md:gap-8">
                  ...
                  {secondaryLinks.map((item) => (
                    <Link
                      key={item.name}
                      to={item.path}
                      className="text-sm md:text-base font-sans font-medium text-texto hover:text-cine-gold transition-colors"
                    >
                      {item.name}
                    </Link>
                  ))}
                ...
                {ultimasEntradas.slice(0, 3).map((entrada, idx) => (
                  <Link key={idx} to="/ensayos" className="group flex items-center gap-6 cursor-pointer">
                    <div className="w-16 h-16 md:w-20 md:h-20 bg-texto/10 rounded overflow-hidden flex-shrink-0">
                      <img src={entrada.image || entrada.img} alt={entrada.title} className="w-full h-full object-cover opacity-80 group-hover:opacity-100 transition-[opacity,transform,filter] group-hover:scale-105 duration-500 img-theme-aware" />
                    </div>
                    <div className="flex flex-col flex-grow">
                      <span className="text-texto/60 text-xs md:text-sm font-sans font-medium mb-1">
                        {entrada.date || entrada.category}
                      </span>
                      {/* TÍTULO ENSAYO (Footer): Usamos 'text-titulo' */}
                      <h4 className="text-titulo text-sm md:text-base font-serif font-medium leading-tight group-hover:text-cine-red transition-colors duration-700">
                        {entrada.title}
                      </h4>
                    </div>
            ...
            <div className="w-full md:w-auto">
              ...
              <div className="relative">
                <input 
                  ...
                  className="w-full md:w-72 bg-transparent border border-texto/30 text-texto rounded-full px-6 py-3 text-sm font-sans focus:outline-none focus:border-cine-blue transition-colors placeholder:text-texto/30"
                />
              </div>
            </div>

            <div className="flex flex-wrap gap-6 text-sm font-sans font-medium text-texto">
              <a href="#" className="hover:text-cine-blue transition-colors">X (Twitter)</a>
              <a href="#" className="hover:text-cine-blue transition-colors">LinkedIn</a>
              <a href="#" className="hover:text-cine-blue transition-colors">Instagram</a>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center justify-center flex-col gap-4 mt-24 pt-12 border-t border-texto/10 max-w-7xl mx-auto w-full">
          {/* TÍTULO FOOTER: Usamos 'text-titulo' para que cambie dinámicamente */}
          <h1 className="font-serif font-black text-titulo text-[12vw] tracking-tighter uppercase leading-none opacity-10 hover:opacity-100 transition-all duration-1000 cursor-default">
            Clase Z.
          </h1>
        </div>
      </footer>
